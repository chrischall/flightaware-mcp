...
export function registerFlightTools(server: McpServer): void {
  server.registerTool(
    "fa_get_flights",
    {
      ...
      inputSchema: z.object({
        ...
        ident: FlightIdent.describe(
          "Flight designator, registration, or fa_flight_id",
        ),
        ident_type: z
          .enum(["designator", "registration", "fa_flight_id"])
          .optional()
          .describe("Disambiguate how `ident` is interpreted"),
        ...dateWindowParams,
        ...pageParams,
      }),
    },
    async ({ ident, ident_type, start, end, max_pages, cursor, view }) => {
      const data = await client.get(
        `/flights/${ident}${qs({ ident_type, start, end, max_pages, cursor })}`,
      );
    ...
    "fa_search_flights",
    {
      ...
      inputSchema: z.object({
        ...
        query: z
          .string()
          .min(1)
          .describe('Simplified "-key value" search string (max 1000 bytes)'),
        ...pageParams,
      }),
    },
    async ({ query, max_pages, cursor, view }) => {
      const data = await client.get(
        `/flights/search${qs({ query, max_pages, cursor })}`,
      );
      return viewResponse(view, data);
    },
  );
  ...
  server.registerTool(
    "fa_search_flights_advanced",
    {
      ...
      inputSchema: z.object({
        ...
        query: z
          ...
          .min(1)
          .describe(
            'Structured "{operator key value}" query, e.g. {match ident UAL*} {> alt 300}',
          ),
        ...pageParams,
      }),
    },
    async ({ query, max_pages, cursor, view }) => {
      const data = await client.get(
        `/flights/search/advanced${qs({ query, max_pages, cursor })}`,
      );
      return viewResponse(view, data);
    },
  );
    ...
    "fa_get_flight_track",
    {
      ...
      inputSchema: z.object({
        ...
        include_estimated_positions: z
          .boolean()
          .optional()
          .describe("Include estimated positions where actual data is missing"),
      }),
    },
    async ({ id, include_estimated_positions, view }) => {
      const data = await client.get(
        `/flights/${id}/track${qs({ include_estimated_positions })}`,
      );
      return viewResponse(view, data);
    },
  );
    ...
    "fa_get_flight_map",
    {
      ...
      inputSchema: z.object({
        ...
        show_data_block: z
          .boolean()
          .optional()
          .describe("Overlay the flight data block on the map"),
        output_dir: z
          .string()
          .optional()
          .describe(
            "Directory to write the PNG to (default: $AEROAPI_OUTPUT_DIR or cwd)",
          ),
        inline: z
          .boolean()
          .optional()
          .describe(
            "Return the PNG inline as base64 instead of writing to disk",
          ),
      }),
    },
    async ({
      ...
      const data = await client.get<{ map?: string }>(
        `/flights/${id}/map${qs({ height, width, show_data_block })}`,
      );
      const base64 = data.map;
      if (!base64) {
        throw new McpToolError(
          "AeroAPI returned no map image for this flight",
          {
  ...
  server.registerTool(
    ...
    async ({ ident, ident_type, start, end, max_pages, cursor, view }) => {
      const data = await client.get(
        `/history/flights/${ident}${qs({ ident_type, start, end, max_pages, cursor })}`,
      );
      return viewResponse(view, data);
    },
  );
  ...
  server.registerTool(
    ...
    async ({ query, max_pages, cursor, view }) => {
      const data = await client.get(
        `/flights/search/positions${qs({ query, max_pages, cursor })}`,
      );
      return viewResponse(view, data);
    },
  );
    ...
    "fa_count_flights",
    {
      ...
      inputSchema: z.object({
        ...
        query: z
          ...
          .min(1)
          .describe(
            'Simplified "-key value" search string (same syntax as fa_search_flights), e.g. -airline UAL',
          ),
      }),
    },
    async ({ query, view }) => {
  ...
  server.registerTool(
    "fa_resolve_flight",
    {
      ...
      inputSchema: z.object({
        view: viewArg(),
        ident: FlightIdent.describe(
          "Flight designator or registration to canonicalize",
        ),
        ident_type: z
          .enum(["designator", "registration", "fa_flight_id"])
          .optional()
    ...
    async ({ ident, ident_type, view }) => {
      const data = await client.get(
        `/flights/${ident}/canonical${qs({ ident_type })}`,
        { cache: "static" },
      );
      return viewResponse(view, data);
    },
