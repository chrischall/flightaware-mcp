...
const TIER =
  " Requires a Standard or Premium AeroAPI tier (the free Personal tier returns 401).";

/** The mutable fields of a flight alert (shared by create + update). */
const alertConfigSchema = {
  ident: FlightIdent.optional().describe(
    "Flight ident / designator to watch (e.g. UAL123)",
  ),
  ...
  aircraft_type: z
    .string()
    .optional()
    .describe("ICAO aircraft type filter (e.g. B738)"),
  start_date: z
    .string()
    .optional()
    .describe("ISO-8601 date the alert becomes active"),
  ...
  max_weekly: z
    ...
    .min(0)
    .optional()
    .describe("Cap on notifications per week"),
...
function buildAlertBody(
  ...
  const { eta, arrival, departure, cancelled, diverted, filed, hold, ...rest } =
...
export function registerAlertTools(server: McpServer): void {
  server.registerTool(
    ...
    {
      description:
        "List the flight alerts configured on your AeroAPI account." + TIER,
    ...
    "fa_set_alerts_endpoint",
    {
      ...
      inputSchema: z.object({
        url: z
          .string()
          .url()
          .describe("HTTPS URL AeroAPI will POST alert payloads to"),
        format: z
          .enum(["json", "json/post", "xml"])
          .optional()
          .describe("Delivery payload format"),
        confirm: schemaConfirm,
      }),
    },
